...
collect_rhdh_info_from_running_pods() {
  ...
  # TODO(asoro): Iterate over all running pods
  local running_pod=$(
  ...
  # Running user ID
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- id 2>/dev/null" "$output_dir/app-container-userid.txt" "id inside the main container"

  # Build Metadata to extract the RHDH version information
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- cat /opt/app-root/src/backstage.json 2>/dev/null" "$output_dir/backstage.json" "backstage.json"
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- cat /opt/app-root/src/packages/app/src/build-metadata.json 2>/dev/null | jq '.card'" "$output_dir/build-metadata.json" "build metadata"

  # Node version
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- node --version 2>/dev/null" "$output_dir/node-version.txt" "Node version"

  # dynamic-plugins-root on the filesystem
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- ls -lhrta dynamic-plugins-root 2>/dev/null" "$output_dir/dynamic-plugins-root.fs.txt" "dynamic-plugins-root dir on the filesystem"

  # app-config generated by the dynamic plugins installer (init container)
  safe_exec "$KUBECTL_CMD -n '$ns' exec '$running_pod' -- cat /opt/app-root/src/dynamic-plugins-root/app-config.dynamic-plugins.yaml 2>/dev/null" "$output_dir/app-config.dynamic-plugins.yaml" "app-config.dynamic-plugins.yaml file"

  # Collect all running processes from all containers in the pod
  # Use || true to ensure process collection failure doesn't stop the rest of data collection
  collect_container_processes "$ns" "$running_pod" "$output_dir" || true
}
    ...
    for container in $containers; do
      ...
      if [[ "$copied" != "true" ]]; then
        ...
        log_info "The application is not instrumented to generate heap dumps on SIGUSR2"
        {
          ...
          echo ""
          echo "   oc adm must-gather --image=quay.io/rhdh-community/rhdh-must-gather:next -- \\"
